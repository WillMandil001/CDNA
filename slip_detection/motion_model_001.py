...
# ==============================================================================================
''' This model will use the motion representation instead of the image representation == 1 channel image shearx and sheary '''
import math
import types
import random
import subprocess
...
def load_and_concat_examples(batch, process_channel):
    ...
    for i in xrange(0, len(img_training_set)):
        image_set = np.float32(np.load(img_training_set[i]).astype(np.uint8)*255)
        images.append([np.asarray([image]) for image in image_set])
    ...
    sta_training_set = [np.squeeze(sta, axis=1) for sta in sta_training_set]
    img_training_set = np.split(ary=img_training_set, indices_or_sections=img_training_set.shape[1], axis=1)
    # remove second singel channel for image (should be:: sequence length , batch , channel , height , width)
    img_training_set = np.asarray(img_training_set)[:, :, 0]
...
class Model(chainer.Chain):
    ...
    def __init__(self, num_masks, is_cdna=True, use_state=True, scheduled_sampling_k=-1, num_frame_before_prediction=2, prefix=None, process_channel=0):
        ...
        with self.init_scope():
            ...
            self.current_state = L.Linear(3)  # change this from 5 to 7?
...
@click.option('--train_test_split', type=click.FLOAT, default=0.9, help='The percentage of data to use for the training set, vs. the test set. Set as 0 for no test set.')
